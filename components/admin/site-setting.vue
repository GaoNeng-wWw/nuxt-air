...
const { prevent = false } = defineProps<{
  prevent?: boolean;
}>();
const emits = defineEmits<{
  submitSuccess: [PublicSiteInfo];
  submitFail: [];
}>();
  ...
  if (prevent) {
    ...
    avatarUrl.then((url) => {
      emits('submitSuccess', {
        ...value,
        ownerAvatar: url,
        social: value.social ? value.social as unknown as { _id: string;icon: string;url: string } : [],
      });
    });
    return;
  }
