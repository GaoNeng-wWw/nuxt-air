...
const props = defineProps<DialogContentProps & { class?: HTMLAttributes['class']; x?: number; y?: number; pure?: boolean }>();
      ...
      <dialog-content
        ...
        :class="
          cn(
            ...
            `,
            props.pure ? 'bg-transparent! size-fit p-0 outline-none border-none' : '',
            props.class,
          )"
      >
        <slot />
        <dialog-close
          v-if="!pure"
          class="w-fit h-fit text-foreground fixed top-4 right-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none"
        >
